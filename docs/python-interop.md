...
| Built-in combinator surface on `PyStream` (legacy `test_streams` parity plus wingfoil-only additions) | `map`/`filter`/`merge`/`merge_all`/`delay`/`distinct`/`count`/`limit`/`skip`/`skip_while`/`step_by`/`take_while`/`throttle`/`sample`/`difference`/`pairwise`/`enumerate`/`not`/`inspect`/`accumulate`/`buffer`/`window`/`with_time`/`collect`/`fold`/`reduce`/`filter_map`/`filter_value`/`filter_none`/`sum`/`mean`/`average`/`bimap`/`split`/`dataframe` — all with Python-exception propagation; `Vec`→list & `(nanos,value)` tuple edge conversions | ✅ done (`graph.rs`, `python.rs`; PRs #549/#551 + follow-ups) |
