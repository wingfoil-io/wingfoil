...
1. **statistics** — pure computation, the largest single chunk, huge test
   ...
   incremental-moment variants, `tests/statistics_rolling.rs`); and
   **time-windowed** rolling (`TimeWindowedSum`/`Mean`/`Min`/`Max`/`Var`/`Std`/
   `Median` over a bounded `Window::Time`, count-weighted — incremental
