...
| C4 | **Compiled-path IO ingestion** — ~~busy-poll (`ALWAYS`) sources~~ + bursts are not expressible in `compiled()`/`nitro!`. | ⚪ | **Half resolved (#758).** The gating decision `port-plan.md` framed — *wake channel or busy-spin?* — was taken on the busy-spin side and shipped: `Poll` carries `#[op(build = poll, no_builder)]`, `expand_compiled` const-folds `always` out of its ops' `ACTIVATION`s into `Kernel::set_spin`, historical runs are rejected, and a `nested()` island declares `always` outward so the *outer* engine spins. Three-tier parity in `tests/poll_all_tiers.rs`. **Busy-poll ingest also generates**: `Builder::poll` records its build name by hand (`no_builder` suppresses the generated builder, and with it the usual `set_node_build`), so a config-driven ingest graph emits through `codegen` like any other — same file. **Still excluded: wake-driven ingest** (`external`/`channel`, `Activation::THREADED`) and bursts. Two independent blockers, only one small: `compiled()` builds its `Kernel` without a ready receiver, so a THREADED node's dirty bit is never set (a near-copy of the `spins` fold would fix it); and `external`/`channel` are hand-written `Builder` methods returning producer handles that must escape *before* the run, while `compiled()` returns only when it ends — so it wants a scoped entry point, not a signature tweak. Note the two ingest styles are **not** two sources: `begin_cycle` drains external wake-ups before it branches on `spin`, so one `channel` wiring serves the deterministic historical test, the parked realtime run and the spinning one (`tests/waked_source_under_spin.rs`). That makes the residue a dev/deploy *usability* gap rather than the parity non-issue the cutover ruling addressed. (was #502/#503) **Cutover ruling 2026-08-03: stays post-v1** (cutover-plan 2.8) — still correct as a cutover call, since legacy has no compiled tier for any of this to regress against. |
