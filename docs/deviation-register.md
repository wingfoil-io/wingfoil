...
| C6 | **`Graph::export` (GML topology dump) not ported.** Classic writes the wired topology to a GML file (`wingfoil/src/graph.rs:1207`, test `graph_export_writes_gml_file`); next's `Builder` has no export at all. | ⚪ | **Deliberately dropped, not deferred-with-intent-to-port.** Port-plan Phase 5 rules it out by name: we want a better introspection/visualization story than a one-off GML dump, designed and scoped separately, rather than a same-shape port of a debug-only helper. Nothing in the engine blocks it — `Builder` holds the full topology plus debug labels — so a future introspection surface can reintroduce it cheaply. **Needs an explicit cutover ruling** (accept the drop, or port `export` before the swap) since it is a public classic API. `port-plan.md` Phase 5 "Graph export". |
| C7 | **Latency ops are fluent/interpreted-only** — `stamp` / `stamp_precise` / `latency_report` have no `nitro!` / `compiled()` / `nested()` form. | ⚪ | **Classic-parity in practice, out of scope by design.** Classic exposes latency solely through `LatencyStreamOps` (there is no other path to port), so the *user-facing* surface matches. The gap is next-internal: a stamp's stage is a compile-time **type** parameter (`stamp::<quote_latency::produce>()`), which does not map onto the `nitro!` value-dispatch table — the macro forwards *values* to `Cfg`, and a type argument has no value to forward. Closing it would need per-op type-argument syntax in `nitro!`, which no other op family wants. Compiled islands can still carry latency-bearing payloads; only the stamping ops themselves must sit on the interpreted side. `src/latency.rs` "Deviation from classic"; `port-plan.md` Phase 5 "Latency". |
...
1. **C6** — `Graph::export` is a *public classic API* we have chosen not to
   ...
   the swap); it is the only classic public function next has no answer for.
