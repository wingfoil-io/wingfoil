...
| C2 | **zmq cross-language interop not ported.** | ⚪ | The `bincode` wire envelope is next-local; not wire-compatible with a legacy/Python peer. Deferred with the Python bindings (Phase 6). **Cutover ruling 2026-08-03: close the gap** — the deferral has expired now Phase 6 is done, and cross-language interop is a legacy-parity capability rather than superset work. Own PR; `cutover-plan.md` row 2.3. `adapters/zmq.rs`. |
...
| C7 | **Latency ops are fluent/interpreted-only** — `stamp` / `stamp_precise` / `latency_report` have no `nitro!` / `compiled()` / `nested()` form. | ⚪ | **Legacy-parity in practice, out of scope by design.** Legacy exposes latency solely through `LatencyStreamOps` (there is no other path to port), so the *user-facing* surface matches. The gap is next-internal: a stamp's stage is a compile-time **type** parameter (`stamp::<quote_latency::produce>()`), which does not map onto the `nitro!` value-dispatch table — the macro forwards *values* to `Cfg`, and a type argument has no value to forward. Closing it would need per-op type-argument syntax in `nitro!`, which no other op family wants. **Cutover ruling 2026-08-03: close it anyway** — the tier surface should be uniform across the op catalog rather than carrying one hand-excepted family, so `nitro!` grows the type-argument syntax. Own PR; `cutover-plan.md` row 2.2. Compiled islands can still carry latency-bearing payloads; only the stamping ops themselves must sit on the interpreted side. `src/latency.rs` "Deviation from legacy"; `port-plan.md` Phase 5 "Latency". |
