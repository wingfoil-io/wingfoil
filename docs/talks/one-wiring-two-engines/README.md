...
| 7 | 91,997 / 15,387 / 324 / 46 | the counts the two runs print. These are deterministic; the example's own [README](../../../crates/wingfoil/examples/core/top_of_book/README.md) explains why the wall-clock line is **not**, which is why no millisecond figure is pinned on the slide |
| 7 | the pacing of the live panel | the captured epoch timestamps' real gaps, slowed 12× so the pauses are visible — stated on the slide itself |
...
| 8 | the three patterns, and the example named for each | busy loop = `Activation::ALWAYS` ([`iceoryx2/read.rs`](../../../crates/wingfoil/src/adapters/iceoryx2/read.rs), also `aeron/read.rs` and `fix.rs`); worker thread = [`source_at_start`](../../../crates/wingfoil/src/fluent.rs) (`kafka.rs`); async = [`produce_async`](../../../crates/wingfoil/src/async_source.rs) (`postgres.rs`) |
