...
| 10 | the three costs Nitro factors out | interpreted drives one `CycleFn = Box<dyn FnMut(&mut Kernel) -> Result<bool>>` per node off a sparse dirty-list ([`interp.rs`](../../../crates/wingfoil/src/interp.rs)); the compiled emission replaces each with a local and straight-line topological order (slide 12 shows it) |
