...
| Trivial state/closure | ✅ distinct, drop_small_change, difference, pairwise, enumerate, limit, skip, map_filter, throttle, inspect, logged, window, buffer, with_time, ticked_at/-elapsed, not, print, timed (`tests/catalog.rs`, `tests/catalog_ops.rs`, `tests/catalog_flow.rs`); ✅ split, combine, collapse (Burst/tuple structural, `tests/catalog_flow.rs`) | recipe proven; `skip` is new surface with no legacy twin and has three-engine parity in `tests/compiled_stateful_ops.rs`; `pairwise` likewise — it is `difference` without the `Sub` bound, tupling where `difference` subtracts, so it reaches types with no arithmetic; `enumerate` adds the zero-based per-stream context no composition of the existing ops recovers without an extra node and a one-based correction (`tests/catalog.rs`, three-engine and rerun parity in `tests/compiled_stateful_ops.rs` / `tests/rerun.rs`). `window`/`buffer` use `Ctx::is_last_cycle`; `combine` builds the burst locally (no shared-cell port). `print` prints per-tick (deviation D8, dropping legacy's teardown buffer); `logged` is fluent-only (deviation D9) — `&str` label vs `String` cfg. `not` and `collapse` were fluent-only sugar (over `map` / `map_filter`) until promoted to real ops (`ops::Not` / `ops::Collapse`, `tests/catalog_not_collapse.rs`); both are dual-mode now, exercised in `op_completeness.rs::surface_sugar`. `split` stays fluent-only and cannot be promoted — two output streams against an `Op`'s single `Out` |
