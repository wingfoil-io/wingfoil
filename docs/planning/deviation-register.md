...
| A8 | **A historical `RunFor::Forever` run ends when every `channel` receiver has drained**, even while another source (`ticker`) is still scheduling. | 🟡 | **Deliberate, and the reason it is worth a row is that it cuts against the "end when the callback queue empties" rule:** legacy — and wingfoil before this — ended a historical run only when its scheduler had nothing left, so a graph with a `channel` feed *and* a `ticker` kept ticking after the data stopped, with engine time walking to `NanoTime` overflow (#978). The run now ends once every **self-driven** receiver has reached end-of-stream *and* no callback the feeds can still activate is pending, so `delay` / `feedback` / downstream work drains first and only a heartbeat source is ignored. **Bounded runs are untouched:** the end is armed only under `RunFor::Forever`, so `RunFor::Duration` / `Cycles` keep their explicit tail (the bounded backtest's end-of-day work, per #978's own "what we do instead today"). Realtime keeps first-close-wins (its waker-driven model), and a graph with a channel that never ends still does not end. `interp.rs::report_channel_done` / `Runner::channel_feeds_exhausted`; `tests/channel.rs` (`channel_eof_ends_a_historical_run_with_another_source`, `historical_run_waits_for_every_channel_before_it_ends`, `pending_delay_drains_before_a_historical_run_ends`, `pending_feedback_drains_before_a_historical_run_ends`, `explicit_duration_keeps_its_tail_after_the_data_stops`). |
