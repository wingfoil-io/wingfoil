...
position and mid. These live in the `wingfoil` crate itself, as
feature-gated modules under `adapters/execution/`, while venue execution
...
dependencies and release cadence, which a venue client has and a simulator
does not — and every crate split in this tree exists for a hard technical
reason (proc-macro, cdylib, a wasm target) that a simulator does not have
either.
