...
Five kinds of document live here, and the split is by audience.
...
| [**talks/**](talks/) | Conference and meetup decks — self-contained, offline, one directory each |
...
[`blog/`](blog/), which carries its own entry criteria. A deck prepared for a
real audience goes in [`talks/`](talks/), which carries its own too — chiefly
that a talk records where every number on its slides came from. For everything
else:
