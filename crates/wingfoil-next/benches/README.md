...
- **next-specific** — the tier/engine suites that have no classic counterpart
  (`tiers`, `custom_op`, `store_baseline`);
- **ports of `wingfoil/benches/`** — one target per classic target, with the
  same name, the same `required-features` gating and, wherever possible, the
  same workload, so a next reading can be put straight beside the classic one.
  That comparability is the whole point of the ports, and it disappears at the
  Phase-7 cutover when the classic bar goes away.
...
| Target | Features needed | Classic twin | What it measures |
|---|---|---|---|
| `tiers` | — | *(next-only)* | classic / interpreted / compiled / nested, side by side, on eight workloads |
...
harness the graph benches drive — the twin of classic's `bench`-gated
...
- The four `aeron_*` benches and `iceoryx2` drive ported *backend* / value
  ...
  *same types* (next re-exports classic's), so `iceoryx2` and `nanotime` measure
...
- `graph`, `bfs_vs_dfs_wingfoil` and `iceoryx2_modes` genuinely move onto the
  ...
  `T`. Node counts match the classic graphs exactly, so the numbers stay
  comparable.

## Report output

`cargo bench` writes criterion's HTML report to `target/criterion/`. Classic
captured one such report for its `10x10` graph run under
[`wingfoil/benches/images/`](../../../../wingfoil/benches/images/), with the
statistics table and plot commentary in
[`wingfoil/benches/README.md`](../../../../wingfoil/benches/README.md). Those are
