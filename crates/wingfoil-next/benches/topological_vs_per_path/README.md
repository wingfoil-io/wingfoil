...
|------|-----------|---------|
| [wingfoil.rs](wingfoil.rs) | wingfoil-next | `s.join(&s, \|a, b\| a + b)` per level, two `nitro!` blocks per depth — one taking a trigger (interpreted + island, per tick), one self-contained (all three tiers, per cycle) |
