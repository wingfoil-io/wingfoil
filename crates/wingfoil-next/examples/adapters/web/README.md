# web Adapter Example (wingfoil-next)
...
```sh
cargo run -p wingfoil-next --example web_adapter --features web
```

The server binds to `127.0.0.1:8080` by default. Override with:

```sh
WINGFOIL_WEB_ADDR=0.0.0.0:9000 cargo run -p wingfoil-next --example web_adapter --features web
```
...
```sh
WINGFOIL_WEB_HISTORICAL=1 cargo run -p wingfoil-next --example web_adapter --features web
```
...
```
wingfoil-next web example listening on ws://127.0.0.1:8080/ws  (codec: Bincode, mode: realtime)
