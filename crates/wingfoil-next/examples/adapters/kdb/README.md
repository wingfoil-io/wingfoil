# KDB+ Adapter Examples (wingfoil-next)
...
```sh
cargo run -p wingfoil-next --example kdb_read         --features kdb
cargo run -p wingfoil-next --example kdb_read_cached  --features kdb
