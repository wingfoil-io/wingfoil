...
//! example binaries.
//!
//! A port of classic `wingfoil/examples/latency/shared.rs` onto the next
//! engine. The data layer is unchanged — `Traced` and `latency_stages!` are
//! engine-agnostic and re-exported from the classic crate by
//! `wingfoil_next::latency` — so this file is byte-for-byte the classic one
...
//!
//! # Deviation from classic: the `#[type_name(...)]` overrides are required
//!
//! Both payload types below pin an explicit iceoryx2 `type_name`, which the
//! classic example omits. Without them the default `ZeroCopySend::type_name()`
...
//!
//! Classic documents this exact hazard on its `Traced<T, L>` `ZeroCopySend`
//! impl (`wingfoil/src/latency.rs`) and provides `#[type_name(...)]` to escape
//! it, but its own `examples/latency` never applies it — so the classic pair
