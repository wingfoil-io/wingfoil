...
Prerequisites:
  ...
  cargo build --release -p wingfoil-next --example latency_e2e_ws_server \\
      --features "web-tls,iceoryx2,prometheus,otlp"
  cargo build --release -p wingfoil-next --example latency_e2e_fix_gw \\
...
EXAMPLE_DIR = HERE.parent.parent  # crates/wingfoil-next/examples/showcase/latency_e2e
...
for path, hint in [
    (WS_SERVER_BIN, "cargo build --release -p wingfoil-next --example latency_e2e_ws_server --features 'web,iceoryx2,prometheus,otlp'"),
    (FIX_GW_BIN, "cargo build --release -p wingfoil-next --example latency_e2e_fix_gw --features 'fix,iceoryx2'"),
]:
    if not path.exists():
...
# ── Spend cap ────────────────────────────────────────────────────────────
# One AWS Budget, two emails: 80%% forecast (warning) and 100%% actual
# (breach). The real cost limiter is the idle-stop daemon on the box;
# this is just the backstop that surfaces "something's wrong" before
# month-end.
#
# Skipped entirely if alert_email is unset — no point in a budget with
# nowhere to deliver the email.
