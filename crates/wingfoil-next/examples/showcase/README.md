...
```sh
cargo run -p wingfoil-next --release --example latency_sub --features iceoryx2
cargo run -p wingfoil-next --release --example latency_pub --features iceoryx2
```
...
```sh
docker compose -f next/crates/wingfoil-next/examples/showcase/latency_e2e/docker-compose.yml up -d
```
