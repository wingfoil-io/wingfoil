...
```bash
cargo test -p wingfoil-next --features fluvio --test fluvio_adapter
cargo test -p wingfoil-next --features fluvio-integration-test -- --test-threads=1
```
...
`wingfoil-next-python` feature `fluvio = ["wingfoil-next/fluvio", "_common"]`.
