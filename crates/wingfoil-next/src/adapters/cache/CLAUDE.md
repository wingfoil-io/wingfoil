...
```bash
cargo test -p wingfoil-next --features cache --test cache_adapter
```
...
variant; `wingfoil-next-python` has no `cache` feature.
...
cargo lint
cargo lint-all
cargo test -p wingfoil-next --features cache
```
