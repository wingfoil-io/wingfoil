# PostgreSQL Adapter (wingfoil-next)
...
```bash
cargo test -p wingfoil-next --features postgres --test postgres_adapter
cargo test -p wingfoil-next --features postgres-integration-test -- --test-threads=1
```
