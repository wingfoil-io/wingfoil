...
```bash
cargo test -p wingfoil-next --features augurs --test augurs_adapter
```
...
`wingfoil-next-python` feature `augurs = ["wingfoil-next/augurs", "_common"]`.
...
cargo lint
cargo lint-all
cargo test -p wingfoil-next --features augurs
```
