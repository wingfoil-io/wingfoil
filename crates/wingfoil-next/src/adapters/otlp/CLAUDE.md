...
```bash
cargo test -p wingfoil-next --features otlp --test otlp_adapter
cargo test -p wingfoil-next --features otlp-integration-test -- --test-threads=1
```
...
`wingfoil-next-python` feature `otlp = ["wingfoil-next/otlp", "_common"]`.
