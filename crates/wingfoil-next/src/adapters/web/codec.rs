...
mod tests {
    ...
    fn bincode_envelope_roundtrip() {
        ...
        let bytes = CodecKind::Bincode.encode(&env).expect("codec round trip");
        let back: Envelope = CodecKind::Bincode.decode(&bytes).expect("codec round trip");
        assert_eq!(env, back);
    }
    ...
    fn json_envelope_roundtrip() {
        ...
        let bytes = CodecKind::Json.encode(&env).expect("codec round trip");
        let back: Envelope = CodecKind::Json.decode(&bytes).expect("codec round trip");
        assert_eq!(env, back);
    }
    ...
    fn control_message_hello_roundtrip() {
        ...
        let bytes = CodecKind::Bincode.encode(&ctrl).expect("codec round trip");
        let back: ControlMessage = CodecKind::Bincode.decode(&bytes).expect("codec round trip");
        assert_eq!(ctrl, back);
    }
    ...
    fn control_message_subscribe_roundtrip() {
        ...
        let bytes = CodecKind::Json.encode(&ctrl).expect("codec round trip");
        let back: ControlMessage = CodecKind::Json.decode(&bytes).expect("codec round trip");
        assert_eq!(ctrl, back);
    }
    ...
    fn control_message_complete_roundtrip() {
        ...
        for codec in [CodecKind::Bincode, CodecKind::Json] {
            let bytes = codec.encode(&ctrl).expect("codec round trip");
            let back: ControlMessage = codec.decode(&bytes).expect("codec round trip");
            assert_eq!(ctrl, back);
        }
    }
    ...
    fn control_message_existing_variants_keep_wire_layout() {
        ...
        let bytes = CodecKind::Bincode.encode(&hello).expect("codec round trip");
        assert_eq!(bytes[0..4], [0, 0, 0, 0], "Hello must stay variant 0");
        let sub = ControlMessage::Subscribe { topics: vec![] };
        let bytes = CodecKind::Bincode.encode(&sub).expect("codec round trip");
