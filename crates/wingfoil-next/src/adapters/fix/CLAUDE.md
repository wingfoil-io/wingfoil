...
```bash
cargo test -p wingfoil-next --features fix --test fix_adapter
cargo test -p wingfoil-next --features fix-integration-test -- --test-threads=1
```
...
`wingfoil-next-python` feature `fix = ["wingfoil-next/fix", "_common"]`. **In
...
cargo lint
cargo lint-all
cargo test -p wingfoil-next --features fix
cargo test -p wingfoil-next --features fix-integration-test -- --test-threads=1
```
