...
//! and the `wingfoil_next` Python bindings on the other, in a separate process.
//!
...
//! with legacy's `wingfoil` module swapped for `wingfoil_next` and its
...
//! `maturin develop` in `crates/wingfoil-next-python` first. A missing module is
//! a **failure**, never a skip: a silently-skipped interop test is how a broken
//! binding reaches a release green.
//!
//! ```sh
//! cd crates/wingfoil-next-python && maturin develop && cd -
//! cargo test -p wingfoil-next --features zmq-cross-lang-test \
...
fn require_python() {
    ...
    assert!(
        ok,
        "zmq-cross-lang-test is enabled but `import wingfoil_next` failed. \
         Run `maturin develop` in crates/wingfoil-next-python first."
    );
}
