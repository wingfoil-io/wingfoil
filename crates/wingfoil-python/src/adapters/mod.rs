...
//! `wingfoil::adapters::*` I/O adapters.
//!
//! Each adapter lives behind a cargo feature of the same name, which turns on
//! the matching `wingfoil` adapter feature, so a wheel only carries the
//! adapters it was built with (`maturin develop -F postgres`). The generated
//! `#[pyfunction]`s are registered in the `wingfoil` `#[pymodule]` under
//! the same `#[cfg]`.
//!
