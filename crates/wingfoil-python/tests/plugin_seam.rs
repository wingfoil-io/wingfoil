...
//! file is compiled as its own crate and touches only wingfoil-python's
...
use pyo3::prelude::*;
use wingfoil::prelude::Stream;
use wingfoil::{NanoTime, RunFor, RunMode};
use wingfoil_python::{Activation, Ctx, Op, PyElement, PyGraph, Tick, pyadapter, pygraph, pyop};

// Compile-level proof that `#[pyop]` works from an *external* crate: the paths
// the macro emits (`::wingfoil_python::...`) resolve here, and the
...
fn build_triple_subgraph(input: &wingfoil::prelude::Stream<f64>) -> wingfoil::prelude::Stream<f64> {
...
trait CountUpOps {
    fn count_up(&self, from: f64) -> wingfoil::prelude::Stream<f64>;
}

#[pyadapter(name = count_up, source)]
impl CountUpOps for wingfoil::prelude::GraphBuilder {
    fn count_up(&self, from: f64) -> wingfoil::prelude::Stream<f64> {
...
trait PyPushSinkOps {
    fn py_push_sink(&self, target: Py<PyAny>) -> wingfoil::prelude::Stream<()>;
}

#[pyadapter(name = py_push_sink)]
impl PyPushSinkOps for wingfoil::prelude::Stream<f64> {
    fn py_push_sink(&self, target: Py<PyAny>) -> wingfoil::prelude::Stream<()> {
        use wingfoil::prelude::StreamOps;
...
trait BurstDoubleOps {
    fn burst_double(&self) -> wingfoil::prelude::Stream<wingfoil::Burst<f64>>;
}

#[pyadapter(name = burst_double, source)]
impl BurstDoubleOps for wingfoil::prelude::GraphBuilder {
    fn burst_double(&self) -> wingfoil::prelude::Stream<wingfoil::Burst<f64>> {
        use wingfoil::prelude::{SourceOps, StreamOps};
        let a = self
...
impl CheckedRampOps for wingfoil::prelude::GraphBuilder {
...
#[pyo3(signature = (start, step = 1.0))]
fn ramp(g: &wingfoil::prelude::GraphBuilder, start: f64, step: f64) -> Stream<f64> {
    use wingfoil::prelude::{SourceOps, StreamOps};
...
fn build_own_ramp(g: &wingfoil::prelude::GraphBuilder) -> Stream<f64> {
    use wingfoil::prelude::{SourceOps, StreamOps};
    g.ticker(Duration::from_nanos(100))
        .count()
...
fn pygraph_multi_output_erases_each_element() {
    ...
    let (lo, hi) = Python::attach(|py| {
        let l = pyo3::Py::new(py, wingfoil_python::Stream::from(counter.clone()))
            .expect("stream pyclass");
        let r = pyo3::Py::new(py, wingfoil_python::Stream::from(other.clone()))
            .expect("stream pyclass");
