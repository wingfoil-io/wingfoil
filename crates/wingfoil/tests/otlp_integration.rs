...
use wingfoil::latency::{Latency, Traced, latency_stages};
...
fn otlp_push_sends_successfully() -> anyhow::Result<()> {
    ...
    let config = OtlpConfig::new(endpoint, "wingfoil-test");

    let g = GraphBuilder::new().with_async_runtime(rt.handle().clone());
    let _sink = g
        .ticker(Duration::from_millis(100))
        .count()
        .otlp_push("wingfoil_integration_counter", config)?;
...
fn otlp_spans_sends_successfully() -> anyhow::Result<()> {
    let (_container, endpoint) = start_collector()?;
    let rt = tokio::runtime::Runtime::new()?;
    let config = OtlpConfig::new(endpoint, "wingfoil-test-spans");
