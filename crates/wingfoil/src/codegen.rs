...
//!     let mut last = None;
//!     for p in periods {
//!         last = Some(g.ticker(p).with_cfg(&p).count().map(double.f).with_src(&double));
//!     }
...
//! - **Stale generation** is possible and nothing here detects it. The
//!   mitigation is that the artifact is `nitro!` *input* — diffable and
//!   reviewable — rather than expanded runner code.
...
///
/// The header exists because the failure this design cannot detect is a
/// **stale** artifact — a file regenerated from yesterday's config, or edited
/// by hand and then overwritten. Saying so at the top of the file is the whole
...
pub fn write_artifact(path: &str, source: &str) -> Result<()> {
    let body = format!(
        "// @generated by wingfoil::codegen — do not edit.\n\
         //\n\
         // Produced by running a wiring function and walking the graph it built,\n\
         // so every config here is frozen at generation time. Regenerate rather\n\
         // than editing; a hand edit is lost on the next run and a stale file is\n\
         // not detected.\n\
         \n\
         {source}\n"
    );
    std::fs::write(path, body).with_context(|| format!("writing generated artifact to {path}"))
}
