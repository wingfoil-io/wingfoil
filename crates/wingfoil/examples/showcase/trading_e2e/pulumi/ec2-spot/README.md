...
Always-on deployment of the trading_e2e demo on a single **t3.small Spot**
...
3. **Five service images** in ECR (or Docker Hub) — see
   `crates/wingfoil/examples/showcase/trading_e2e/Dockerfile.*` and
   `.github/workflows/build-trading-e2e-images.yml`
4. **A baked AMI** — built via the `Build trading_e2e AMI (ec2-spot)` GitHub
...
Trigger `Build trading_e2e AMI (ec2-spot)` from the Actions tab. It runs Packer
...
```bash
cd crates/wingfoil/examples/showcase/trading_e2e/pulumi/ec2-spot/packer
...
```bash
cd crates/wingfoil/examples/showcase/trading_e2e/pulumi/ec2-spot
...
rsync -av --delete \
  crates/wingfoil/examples/showcase/trading_e2e/static/ \
