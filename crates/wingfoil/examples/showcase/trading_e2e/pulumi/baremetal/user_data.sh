#!/bin/bash
# user_data.sh — first-boot bootstrap for the wingfoil trading_e2e bare-metal box.
#
...
if [ "$stage" = "installed" ]; then
  ...
  aws s3 cp   "s3://${BINARIES_BUCKET}/trading_e2e_ws_server" /opt/wingfoil/trading_e2e_ws_server --region "${AWS_REGION}"
  aws s3 cp   "s3://${BINARIES_BUCKET}/trading_e2e_fix_gw"    /opt/wingfoil/trading_e2e_fix_gw    --region "${AWS_REGION}"
  ...
  chmod 0755 /opt/wingfoil/trading_e2e_ws_server /opt/wingfoil/trading_e2e_fix_gw
...
[Unit]
Description=wingfoil trading_e2e ws_server (graph thread pinned to core ${WS_SERVER_CORE})
...
ExecStart=/opt/wingfoil/trading_e2e_ws_server --addr 0.0.0.0:8080 --precise
Restart=on-failure
RestartSec=2
...
[Unit]
Description=wingfoil trading_e2e fix_gw (graph thread pinned to core ${FIX_GW_CORE})
...
ExecStart=/opt/wingfoil/trading_e2e_fix_gw --precise
Restart=on-failure
RestartSec=2
