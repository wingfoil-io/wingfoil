"""
On-demand bare-metal deployment of the wingfoil trading_e2e demo.
...
EXAMPLE_DIR = HERE.parent.parent  # crates/wingfoil/examples/showcase/trading_e2e
WORKSPACE_ROOT = HERE.parents[6]  # repo root
TARGET_RELEASE = WORKSPACE_ROOT / "target" / "release" / "examples"

WS_SERVER_BIN = TARGET_RELEASE / "trading_e2e_ws_server"
FIX_GW_BIN = TARGET_RELEASE / "trading_e2e_fix_gw"
...
aws.s3.BucketObject(
    ...
    bucket=bucket.id,
    key="trading_e2e_ws_server",
    source=pulumi.FileAsset(str(WS_SERVER_BIN)),
)
aws.s3.BucketObject(
    f"{prefix}-bin-fix-gw",
    bucket=bucket.id,
    key="trading_e2e_fix_gw",
