# trading_e2e — bare-metal Pulumi stack
...
```bash
cd crates/wingfoil/examples/showcase/trading_e2e/pulumi/baremetal
...
cargo build --release --manifest-path crates/wingfoil/Cargo.toml --example trading_e2e_ws_server \
    --features "web-tls,iceoryx2,prometheus,otlp"
cargo build --release --manifest-path crates/wingfoil/Cargo.toml --example trading_e2e_fix_gw \
