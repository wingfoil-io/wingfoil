...
| [`odds_evens`](odds_evens/) | — | Split and recombine — a counter fanned out by parity and merged back, in the three explicit steps (wire from a `GraphBuilder`, `build()`, `run(..)`), output via `logged`. |
