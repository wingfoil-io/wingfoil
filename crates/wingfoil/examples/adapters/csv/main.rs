...
fn main() -> anyhow::Result<()> {
    ...
    let input = dir.join("wingfoil_csv_adapter_in.csv");
    let output = dir.join("wingfoil_csv_adapter_out.csv");
    {
    ...
    let _totals_sink = totals.csv_write(dir.join("wingfoil_csv_adapter_totals.csv"))?;
    let mut runner = g.build();
    runner.run(RunMode::HistoricalFrom(NanoTime::ZERO), RunFor::Forever)?;
