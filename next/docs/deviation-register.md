...
| B5 | ~~**`postgres_read` queries all slices at wiring** onto `replay_results` vs classic's lazy `produce_async`.~~ | ✅ | **Resolved (the A1 wiring-side-effect).** `postgres_read` now defers its connect + slice queries to the run via `produce_async` (like classic), so wiring does no I/O and a connection/query failure aborts the run, not graph construction. The window is still validated + sliced at wiring (a pure, fail-fast check); `read_slices` (window-clamp / ordering / decode) is unchanged, its rows fed to `produce_async` as a `(time, record)` stream. **Memory unchanged and *inherently* so:** a historical `produce_async` collects the whole stream up front (the same reason `replay_results` / csv / `postgres_read` are on the unbounded historical path — see "Resolved: channel historical block-collect"), so this is not a memory bound — but it was never a correctness gap ("identical for a bounded run"). `adapters/postgres.rs` deviation #2. |
