...
- **Live-only, reject ratified as-is:** `zmq_sub`, `etcd_sub`, both redis
  ...
  no historical timeline), and `aeron_sub_fragment` (a shared-memory/UDP
  subscription with no addressable past). Nothing to dispatch to under
  historical, so the wiring-time reject is the honest behaviour and B2 is
  accepted for these.
