...
- **kafka**, **fluvio** — candidates *once* they gain a bounded offset-range
  replay reader (the durable log makes this feasible for both; `fluvio_sub`
  already takes a `start_offset`, so it needs only an *end* bound and a
  record-timestamp source). Only the live `_sub` form exists for each today, so
  they stay under ruling (a) until then.
...
| C7 | **Latency ops are fluent/interpreted-only** — `stamp` / `stamp_precise` / `latency_report` have no `graph!` / `compiled()` / `nested()` form. | ⚪ | **Classic-parity in practice, out of scope by design.** Classic exposes latency solely through `LatencyStreamOps` (there is no other path to port), so the *user-facing* surface matches. The gap is next-internal: a stamp's stage is a compile-time **type** parameter (`stamp::<quote_latency::produce>()`), which does not map onto the `graph!` value-dispatch table — the macro forwards *values* to `Cfg`, and a type argument has no value to forward. Closing it would need per-op type-argument syntax in `graph!`, which no other op family wants. Compiled islands can still carry latency-bearing payloads; only the stamping ops themselves must sit on the interpreted side. `src/latency.rs` "Deviation from classic"; `port-plan.md` Phase 5 "Latency". |
