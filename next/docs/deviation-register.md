...
| D11 | **kdb: `kdb_read` keeps classic's `buffer_size` (a no-op in historical mode).** | 🟢 | `kdb_read` is historical-only; the historical `produce_async` collects up front, so the bound is inert — preserved for classic API parity, documented on the factory. The `kdb_write` sink takes a `buffer_size` per D3. |
