...
| `#[pyop]` extensions | **stateful single-input ops done** (`State` is any `Default`-seedable type, re-seeded per run); multi-input shapes + `Cfg`-tuple arg names remain | 🟡 stateful done |
