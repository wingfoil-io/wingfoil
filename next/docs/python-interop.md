...
| `#[pyop]` extensions | **done** — `State` is any `Default`-seedable type (re-seeded per run); `In<'a> = (&A, &B)` emits `module.name(stream, other)` over `wire_op2`, and `(&A, &B, &C)` emits `module.name(stream, second, third)` over the new `wire_op3` / `Builder::register_op3` (the general form of the `Join3`-specialised `trimap`); a tuple `Cfg` destructures into one named Python parameter per element via `arg = (p1, p2, …)`. Arity 4+ needs a `register_op<n>`/`wire_op<n>` pair first — `register_op3` is the template — after which the macro extends by one arm | ✅ done |
