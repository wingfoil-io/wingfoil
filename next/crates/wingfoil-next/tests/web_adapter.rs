...
//! under:
//! ```sh
//! cargo test -p wingfoil-next --features web -- --test-threads=1
//! ```
//!
...
fn pub_round_trip_tls() -> anyhow::Result<()> {
    ...
    let cert_path = std::env::temp_dir().join(format!("wingfoil-next-web-tls-{unique}-cert.pem"));
    let key_path = std::env::temp_dir().join(format!("wingfoil-next-web-tls-{unique}-key.pem"));
