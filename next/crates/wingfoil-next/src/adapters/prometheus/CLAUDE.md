# Prometheus Adapter (wingfoil-next)
...
```bash
cargo test -p wingfoil-next --features prometheus --test prometheus_adapter
cargo test -p wingfoil-next --features prometheus-integration-test -- --test-threads=1
```
...
`prometheus = ["wingfoil-next/prometheus", "_common"]`. **In `all-adapters` and
