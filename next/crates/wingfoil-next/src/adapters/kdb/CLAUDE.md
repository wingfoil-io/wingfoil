...
`use wingfoil_next::adapters::kdb::*;` and never name the submodules.
...
```bash
cargo test -p wingfoil-next --features kdb --test kdb_adapter
...
`wingfoil-next-python` feature `kdb = ["wingfoil-next/kdb", "_common"]`.
