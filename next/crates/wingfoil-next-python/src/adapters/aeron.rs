//! Python bindings for the wingfoil-next **Aeron** adapter
//! ([`wingfoil_next::adapters::aeron`]).
//!
...
) -> Result<impl wingfoil_next::adapters::aeron::AeronSubscriberBackend> {
...
fn publish(
    ...
    stream_id: i32,
    timeout: Duration,
) -> Result<impl wingfoil_next::adapters::aeron::AeronPublisherBackend> {
...
mod tests {
    ...
    fn a_fragment_is_copied_out_whole() {
        ...
        let buffer = FragmentBuffer::new(
            &bytes,
            wingfoil_next::adapters::aeron::FragmentHeader {
                position: 0,
                session_id: 1,
                stream_id: 1001,
            },
