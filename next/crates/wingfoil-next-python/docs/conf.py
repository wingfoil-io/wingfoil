"""Sphinx configuration for the ``wingfoil_next`` Python bindings.

The documented module is ``wingfoil_next`` — a *mixed* maturin layout: the
importable package lives in ``../python/wingfoil_next`` and wraps the compiled
extension ``wingfoil_next._wingfoil`` (see ``../pyproject.toml``). Autodoc
...
* The pure-Python package under ``../python`` is added to ``sys.path`` so a
  ...
  is not enough — ``wingfoil_next/__init__.py`` imports ``._wingfoil``, which
...
def _release() -> str:
    ...
    try:
        ...
        try:
            return version("wingfoil-next-python")
        except PackageNotFoundError:
            pass
    except ImportError:  # pragma: no cover - Python < 3.8
        pass
    try:
        import wingfoil_next

        return getattr(wingfoil_next, "__version__", None) or "0.1.0"
    except ImportError:
        return "0.1.0"
...
try:
    import wingfoil_next  # noqa: F401
