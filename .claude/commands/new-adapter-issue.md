...
0. **Provenance** — the very first line of the issue body must state, verbatim:
   ...
   > _This issue was generated by the `/new-adapter-issue` skill. Any implementation picking it up **must** be validated against the `/new-adapter` skill — every step, file layout, feature flag, test harness, and deliverable in that skill is authoritative, and deviations must be justified in-thread before merging._

   Do not paraphrase or omit this line; it is what signals to future implementers (human or agent) that `/new-adapter` is the normative spec for the work.
...
10. **Deliverables / acceptance criteria** — checklist mirroring the `/new-adapter` steps. The final box is mandatory and enforces the validation contract from the provenance line:
