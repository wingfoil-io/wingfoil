...
use super::seed::{query_seeds, register_with_seeds, start_seed};
...
// --- Seed unit tests (ports 5570–5573) ---

#[test]
fn seed_register_and_lookup() {
    let _seed = start_seed("tcp://127.0.0.1:5570").unwrap();
    std::thread::sleep(Duration::from_millis(50));

    register_with_seeds(
        "quotes",
        "tcp://127.0.0.1:9001",
        &["tcp://127.0.0.1:5570".to_string()],
    )
    .unwrap();
    let addr = query_seeds("quotes", &["tcp://127.0.0.1:5570"]).unwrap();
    assert_eq!(addr, "tcp://127.0.0.1:9001");
}

#[test]
fn seed_lookup_unknown() {
    let _seed = start_seed("tcp://127.0.0.1:5571").unwrap();
    std::thread::sleep(Duration::from_millis(50));

    let err = query_seeds("unknown", &["tcp://127.0.0.1:5571"]).unwrap_err();
    assert!(
...
fn seed_register_overwrites() {
    let _seed = start_seed("tcp://127.0.0.1:5572").unwrap();
    std::thread::sleep(Duration::from_millis(50));

    register_with_seeds(
        "foo",
        "tcp://127.0.0.1:9002",
        &["tcp://127.0.0.1:5572".to_string()],
    )
    .unwrap();
    register_with_seeds(
        "foo",
    ...
    .unwrap();
    let addr = query_seeds("foo", &["tcp://127.0.0.1:5572"]).unwrap();
    assert_eq!(addr, "tcp://127.0.0.1:9003");
}

#[test]
fn seed_drop_shuts_down() {
    let seed = start_seed("tcp://127.0.0.1:5573").unwrap();
    std::thread::sleep(Duration::from_millis(50));

    register_with_seeds(
        "alive",
        "tcp://127.0.0.1:9100",
        &["tcp://127.0.0.1:5573".to_string()],
    )
    .unwrap();

    // Drop joins the thread. If this hangs the test will timeout — that is the failure signal.
    drop(seed);
}
...
// --- Discovery integration tests (ports 5580–5595) ---
...
fn zmq_named_pub_registers_with_seed() {
    ...
    let handle = std::thread::spawn(move || {
        ticker(Duration::from_millis(50))
            .count()
            .zmq_pub(port, ("prices", SeedRegistry::new(&[seed_addr])))
            .run(
                RunMode::RealTime,
...
fn zmq_sub_discover_end_to_end() {
    ...
    std::thread::spawn(move || {
        ticker(Duration::from_millis(50))
            .count()
            .zmq_pub(port, ("counts", SeedRegistry::new(&[seed_addr])))
            .run(RunMode::RealTime, run_for)
    });

    std::thread::sleep(Duration::from_millis(150));

    let (data, _status) = zmq_sub::<u64>(("counts", SeedRegistry::new(&[seed_addr]))).unwrap();
    let recv_node = data.collect().finally(|res, _| {
        let values: Vec<u64> = res.into_iter().flat_map(|item| item.value).collect();
...
fn zmq_sub_discover_multiple_seeds_first_down() {
    ...
    let addr = zmq_sub::<u64>(("widget", SeedRegistry::new(&[dead_seed, live_seed])));
    assert!(
        addr.is_ok(),
...
fn zmq_sub_discover_no_seed_returns_error() {
    let result = zmq_sub::<u64>(("anything", SeedRegistry::new(&["tcp://127.0.0.1:5586"])));
...
fn zmq_sub_discover_name_not_found() {
    ...
    let result = zmq_sub::<u64>(("nonexistent", SeedRegistry::new(&[seed_addr])));
...
fn zmq_named_pub_historical_mode_fails() {
    ...
    let result = ticker(Duration::from_millis(10))
        .count()
        .zmq_pub(5588, ("test", SeedRegistry::new(&["tcp://127.0.0.1:5589"])))
        .run(RunMode::HistoricalFrom(NanoTime::ZERO), RunFor::Forever);
