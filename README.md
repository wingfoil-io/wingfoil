...
| **Project Bypass** — Onload validation, then a raw ef_vi/DPDK source behind an in-tree `RxSource` seam | Ingress, and the wire-to-trade number the benchmarks currently decline to claim | [#957](https://github.com/wingfoil-io/wingfoil/issues/957), designed in [`kernel-bypass-io.md`](docs/planning/proposals/kernel-bypass-io.md). The first rung needs a Solarflare NIC and a measurement run, not a diff |
